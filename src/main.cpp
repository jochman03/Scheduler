...
constexpr auto WINDOW_WIDTH = std::uint32_t{1920};
constexpr auto WINDOW_HEIGHT = std::uint32_t{1080};
...
static bool EnableDarkTitleBar(HWND hwnd, bool enable){
    ...
    // 20 works on many Win10/11 builds, 19 on some older Win10 builds.
    const DWORD DWMWA_USE_IMMERSIVE_DARK_MODE_OLD = 19;
    const DWORD DWMWA_USE_IMMERSIVE_DARK_MODE_NEW = 20;

    HRESULT hr = DwmSetWindowAttribute(
        hwnd, DWMWA_USE_IMMERSIVE_DARK_MODE_NEW, &useDark, sizeof(useDark)
    );
    if (SUCCEEDED(hr)) return true;

    hr = DwmSetWindowAttribute(
        hwnd, DWMWA_USE_IMMERSIVE_DARK_MODE_OLD, &useDark, sizeof(useDark)
    );
    return SUCCEEDED(hr);
}
...
int main(int, char **)
{
    ...
    auto window_obj = std::make_unique<WindowClass>();
